...
import { allUsers, selectUser } from '../../store/user/UserSlice';
import { useSelector } from 'react-redux';
import { useAppDispatch } from '../../store/hook'
import { useNavigate } from 'react-router-dom'
import { Users } from '../../interface/ResponseInterface';
...
export const DashboardTable = () => {
    ...
    const { allUsersData }  = useSelector(selectUser);
    const rowPageOption = [5, 50, 100]
    const [currentRowPerPage, setCurrentRowPerPage] = React.useState(rowPageOption[0]);
    const [currentTableData, setCurrentTableData] = React.useState<Users[] | null>(null);
    ...
    useEffect(() => {
        dispatch(allUsers()).then(data => {
            if(data){
                let userData = data.payload
                let orgArray: any = []
                userData.map((item: Users, index: number) => {
                    orgArray.push(item.orgName)
                })

                setAllOrganizations(orgArray)

                setUserDataLength(userData.length) 
                const selectedData = userData.slice(0, currentRowPerPage);
                setCurrentTableData(selectedData)
                setPagnationLength(userData.length/currentRowPerPage)
                setUserIncomingData(userData)
            }
        });
     ...
     const handleChangeRowsPerPage = (
        ...
        let selectedData = allUsersData.slice(0, newPageRow);
        setCurrentTableData(selectedData)
        setPagnationLength(userIncomingData.length/newPageRow)
    };
    ...
    const handleChangePage = (
        ...
        setPage(newPage)
        const updatedRows = allUsersData.slice(
   ...
   const handlePaginationChange = (event: React.ChangeEvent<unknown> | null,
    ...
    const gotToUserData = () => {
        navigate(`user-detail/${currentUser}`)
    }
    ...
    //  View User Menu
    const [viewUserAnchorEl, setViewUserAnchorEl] = React.useState<null | HTMLElement>(null);
    const openViewUser = Boolean(viewUserAnchorEl);
    const handleViewUserClick = (event: React.MouseEvent<HTMLElement>, index: number) => {
        setCurrentUser(index)
        setViewUserAnchorEl(event.currentTarget);
    };
    const handleViewUserClose = () => {
        setViewUserAnchorEl(null);
    };
...
const handleSelectChange = (event: SelectChangeEvent) => {
  ...
  const resetFilter = () => {
   ...
   /// setCurrentTableData(allUsersData)

   resetDefaultTableData()

  }

  const resetDefaultTableData = () => {
    const selectedData = allUsersData.slice(0, currentRowPerPage);
    setCurrentTableData(selectedData)
  ...
  const handleTableFilter = () => {
   const orgName = filterData.organization !== '' ? '*' + formatText(filterData.organization) : false
   const username = filterData.username !== '' ? '*' + formatText(filterData.username)  : false
   const email = filterData.email !== '' ? '*' + formatText(filterData.email)  : false
   const phone = filterData.phone !== '' ? '*' + formatText(filterData.phone)  : false
   const joinDate = filterData.email !== '' ? '*' + formatText(filterData.joinedDate)  : false


   if(email){
        let tableRes =  filterArray(email, 'email')
        setFilterTableData(filterTableData.concat(tableRes))
   }


   if(orgName){
        let tableRes =  filterArray(orgName, 'organization')
        setFilterTableData(filterTableData.concat(tableRes))
   }

   if(username){
        let tableRes =  filterArray(username, 'username')
        setFilterTableData(filterTableData.concat(tableRes))
    }

    if(phone){
        let tableRes =  filterArray(phone, 'phone')
        setFilterTableData(filterTableData.concat(tableRes))
    }

    if(joinDate){
        let tableRes =  filterArray(joinDate, 'date')
        setFilterTableData(filterTableData.concat(tableRes))
    }


    let removeDups = filterDuplicates(filterTableData)
    setCurrentTableData(removeDups)
    handleFilterClose()
  }



  const filterDuplicates = (data: any) => {
   return data.filter((val: Users, index: number, arr: any) => {
        return arr.indexOf(arr.find((t: any, index: number) => t.id === val.id)) === index;
      });
  }
  ...
  const filterArray = (expre: string, type: string) => {
    var regex = stringToRegExp(expre)

    switch (type) {
        case 'organization':
           return  allUsersData.filter(function (item: any, index: number) {
                return (
                  regex.test(item.organization)
                )
        ...
        case 'phone':
            return allUsersData.filter(function (item: any, index: number) {
                return (
                    regex.test(item.profile.phoneNumber)
                )
                });
            break;
            ...
            <div className='p-2'>
                <div className="mt-3 shadow rounded">
                    ...
                    >
                        ...
                        <div className='p-3 menu-item-wrapper'>
                            <div className="form-group item-input-wrapper">
                                ...
                                <Select className='select-item'
                                    ...
                                    >
                                        <MenuItem className='placerholder menu-content-item' disabled value="">
                                        Organization
                                    </MenuItem>
                                    
                                        {allOrganizations.map((item, index) => {
                                            return  <MenuItem key={index} className='menu-content-item' value={item}>{item}</MenuItem>
                                        })}
                            ...
                            <div className='item-input-wrapper'>
                                ...
                                <Select className='select-item'
                                    ...
                                    placeholder='Status'
                                    >
                                        <MenuItem className='placerholder' disabled value="">
                                        Status
                                    </MenuItem>
                            ...
                            <div className='filter-btn-wrapper'>
                                ...
                                <button onClick={handleTableFilter}>Filter</button>
                            </div>
                        </div>
                    </Menu>
                    ...
                    <TableContainer className='table-container' component={Paper}>
                        <Table sx={{ minWidth: 400 }} aria-label="simple table">
                            <TableHead>
                                <TableRow>
                                    <TableCell className='table-cell-text table-header'>organization
                                        <Tooltip title="Filter list" ref={filterBtnRef}>
                                        <IconButton id="filter-button"
                                                aria-controls={openViewUser ? 'filter-menu' : undefined}
                                                aria-haspopup="true"
                                                aria-expanded={openViewUser ? 'true' : undefined}
                                    ...
                                    <TableCell className='table-cell-text table-header'>Username
                                    ...
                                    <TableCell className='table-cell-text table-header'>Email
                                        <Tooltip title="Filter list">
                                            <IconButton onClick={handleKeyDown}>
                                                <FilterListIcon />
                                            </IconButton>
                                    ...
                                    <TableCell className='table-cell-text table-header'>Phone number
                                        <Tooltip title="Filter list">
                                            <IconButton onClick={handleKeyDown}>
                                                <FilterListIcon />
                                            </IconButton>
                                    ...
                                    <TableCell className='table-cell-text table-header'>Date joined
                                        <Tooltip title="Filter list">
                                            <IconButton onClick={handleKeyDown}>
                                                <FilterListIcon />
                                            </IconButton>
                                    ...
                                    <TableCell className='table-cell-text table-header'>Status
                                        <Tooltip title="Filter list">
                                            <IconButton onClick={handleKeyDown}>
                                                <FilterListIcon />
                                            </IconButton>
                            ...
                            <TableBody>
                                {currentTableData != null && currentTableData.map((user: Users, index: number) => (
                                    <TableRow
                                        key={index}
                                        sx={{ '&:last-child td, &:last-child th': { border: 0 } }}
                                    >
                                        <TableCell className='table-cell-text' size='small' component="th" scope="row">
                                            {user.orgName}
                                        </TableCell>
                                        <TableCell className='table-cell-text' size='small' align="left">{user.userName}</TableCell>
                                        <TableCell className='table-cell-text' size='small' align="left">{user.email}</TableCell>
                                        <TableCell className='table-cell-text' size='small' align="left">{user.phoneNumber}</TableCell>
                                        <TableCell className='table-cell-text' size='small' align="left">{user.createdAt}</TableCell>
                                        <TableCell className='table-cell-text' size='small' align="left">
                                            <Chip className='chip-text' label={user.education.employmentStatus}  />
                                            <IconButton
                                                aria-label="more"
                                                className='viewer-btn'
                                                id={'view-btn-menu-' + index}
                                                aria-controls={openViewUser ? `view-user-menu-${index}` : undefined}
                                                aria-expanded={openViewUser ? 'true' : undefined}
                                                aria-haspopup="true" 
                                                onClick={(e) => { handleViewUserClick(e, index)}}
                                                >
                                                <MoreVertIcon />
                                            </IconButton>
                                        </TableCell>
                                ...
                                <Menu 
                                    id={'view-user-menu'}
                                    aria-labelledby={'view-btn-menu'}
                                    anchorEl={viewUserAnchorEl}
                                    open={openViewUser}
                                    ...
                                    PaperProps={{
                                        ...
                                        >
                                        <MenuItem className='d-flex gap-3' onClick={gotToUserData}>
                                        ...
                                        <MenuItem className='d-flex gap-3' onClick={handleViewUserClose}>
